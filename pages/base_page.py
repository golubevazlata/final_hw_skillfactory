...
class BasePage():
    ...
    # метод find_element ищет элемент и возвращает этот элемент
    def find_element(self, locator, time=10):
        return WebDriverWait(self.browser, time).until(EC.presence_of_element_located(locator),
                                                       message=f"Can't find element by locator {locator}")

    # метод open, чтобы открыть нужную страницу в браузере - с использованием метода get()
    def open(self):
        self.browser.get(self.url)

    # метод open_reg_page, чтобы открыть форму регистрации в браузере - с использованием метода get()
    def open_reg_page(self):
        self.browser.get(self.url)
        self.find_element(AuthPageLocators.AUTH_REGISTER_LINK).click()

    # метод is_element_present - для перехватывания исключения
